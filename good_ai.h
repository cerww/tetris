...
struct flatstacking_ai {
						...
						if (a) {
							...
							if (just_placed_piece == tetris_piece::T) {
								...
								if (thing.lines_cleared &&
									...
									prev_heights[x] < y &&
									(thing.lines_cleared == 1 || prev_heights[x - 1] == y || prev_heights[x + 1] == y)
								) {
	...
	sbo_vector<piece_hard_drop_spot, 16> do_not_place_spots(
		...
		sbo_vector<piece_hard_drop_spot, 16> ret;

		if (next_piece == tetris_piece::T) {
			for (int i = 0; i < 7; ++i) {
				if (heights[i] == heights[i + 1] && heights[i + 1] == heights[i + 2]) {
					ret.push_back(piece_hard_drop_spot{(int8_t)i, 1});
					ret.push_back(piece_hard_drop_spot{(int8_t)i + 2, 3});
					ret.push_back(piece_hard_drop_spot{(int8_t)i + 1, 2});
				}
			}
			...
			if(heights[height_order[9]] - heights[height_order[6]] >=4) {
	...
	double evaluate_board(const tetris_game& game, int total_garbage_sent, int total_lines_cleared, garbage_calculator garb_state, bool full_eval = true) const {
		...
		double ret = total_garbage_sent * total_garbage_sent * (5 - !full_eval * 2.5)
				- std::abs(bumpines)
				- bumpiness_squared * (2 + !full_eval * 1 + !do_tspins * 2)
				- std::abs(covered) * (8 - !full_eval * 2.5)
				- covered_squared * (3 + !full_eval * 1 + !do_tspins * 1)
